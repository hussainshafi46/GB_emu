...
/*
Implementations for all INC instructions
*/
void inc16(GB_CPU *cpu, int r_register) {
    (*cpu->reg16[r_register])++;
    cpu->cpu_cycles++;
    cpu->machine_cycles += 8;
}
void inc8(GB_CPU *cpu, GB_MMU *mmu, int r_register) {
    uint8_t old, result;
